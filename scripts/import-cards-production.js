...
const productionDatabaseUrl = process.argv[2] || process.env.DATABASE_URL;

if (!productionDatabaseUrl) {
  console.error('❌ Please provide DATABASE_URL');
  console.log('\nUsage:');
  console.log('node scripts/import-cards-production.js "postgresql://user:pass@host:port/dbname"');
...
const pool = new Pool({
  ...
  ssl: productionDatabaseUrl.includes('render.com') ? { rejectUnauthorized: false } : false
});
...
async function importCards() {
  ...
  try {
    ...
    console.log(`\n📊 Total cards in database: ${countResult.rows[0].count}`);
